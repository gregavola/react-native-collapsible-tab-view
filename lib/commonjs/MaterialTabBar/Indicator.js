...
const Indicator = _ref => {
  ...
  const stylez = (0, _reactNativeReanimated.useAnimatedStyle)(() => {
    ...
    const transform = [{
      translateX: itemsLayout.length > 1 ? (0, _reactNativeReanimated.interpolate)(indexDecimal.value, itemsLayout.map((_, i) => i), // when in RTL mode, the X value should be inverted
